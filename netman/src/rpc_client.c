...
int NetManRPCRegisterNetworkStack(void)
{
	...
	if(FrameBufferStatus == NULL) FrameBufferStatus = memalign(64, NETMAN_RPC_BLOCK_SIZE_LESSER * sizeof(struct NetManBD));

	if(FrameBufferStatus != NULL)
	{
		memset(UNCACHED_SEG(FrameBufferStatus), 0, NETMAN_RPC_BLOCK_SIZE_LESSER * sizeof(struct NetManBD));
	...
	while(1)
	{
		...
		while((length = NetManTxPacketNext(&payload)) > 0)
		{
			...
			do {
				...
				bdNext = UNCACHED_SEG(&FrameBufferStatus[(IOPFrameBufferWrPtr + 1) % NETMAN_RPC_BLOCK_SIZE_LESSER]);
				...
				IOPFrameBufferWrPtr = (IOPFrameBufferWrPtr + 1) % NETMAN_RPC_BLOCK_SIZE_LESSER;
