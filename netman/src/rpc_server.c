...
int NetManRPCAllocRxBuffers(void)
{
	...
	for(i = 0; i < NETMAN_RPC_BLOCK_SIZE_LESSER; i++)
	{
...
static void *NETMAN_EE_RPC_Handler(int fnum, void *buffer, int NumBytes)
{
	...
	switch(fnum)
	{
		case NETMAN_EE_RPC_FUNC_INIT:
			...
			if(FrameBufferStatus == NULL) FrameBufferStatus = memalign(64, sizeof(struct NetManBD) * NETMAN_RPC_BLOCK_SIZE_LESSER);

			if(FrameBufferStatus != NULL)
			{
				memset(UNCACHED_SEG(FrameBufferStatus), 0, sizeof(struct NetManBD) * NETMAN_RPC_BLOCK_SIZE_LESSER);
				RxBufferRdPtr = 0;
				RxBufferNextRdPtr = 0;
				IsProcessingRx = 0;
...
static void NETMAN_RxThread(void *arg)
{
	...
	while(1)
	{
		...
		RxBufferNextRdPtr = (RxBufferNextRdPtr + 1) % NETMAN_RPC_BLOCK_SIZE_LESSER;
