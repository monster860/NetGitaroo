...
static struct NetManPacketBuffer pbufs[NETMAN_RPC_BLOCK_SIZE_LESSER];
...
int NetManInitRPCClient(void)
{
	...
	if(FrameBuffer == NULL) FrameBuffer = malloc(NETMAN_RPC_BLOCK_SIZE_LESSER * NETMAN_MAX_FRAME_SIZE);
	if(FrameBufferStatus == NULL) FrameBufferStatus = malloc(NETMAN_RPC_BLOCK_SIZE_LESSER * sizeof(struct NetManBD));

	if(FrameBuffer != NULL && FrameBufferStatus != NULL)
	{
		memset(FrameBuffer, 0, NETMAN_RPC_BLOCK_SIZE_LESSER * NETMAN_MAX_FRAME_SIZE);
		memset(FrameBufferStatus, 0, NETMAN_RPC_BLOCK_SIZE_LESSER * sizeof(struct NetManBD));
		EEFrameBufferWrPtr = 0;
		NumFramesInQueue = 0;

		for(i = 0; i < NETMAN_RPC_BLOCK_SIZE_LESSER; i++)	//Mark all descriptors as "in-use", until the EE-side allocates buffers.
...
static void EnQFrame(const void *frame, unsigned int length)
{
	...
	EEFrameBufferWrPtr = (EEFrameBufferWrPtr + 1) % NETMAN_RPC_BLOCK_SIZE_LESSER;
