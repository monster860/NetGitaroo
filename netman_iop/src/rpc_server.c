...
static void DeQTxPacket(void)
{
	...
	IOPFrameBufferRdPtr = (IOPFrameBufferRdPtr + 1) % NETMAN_RPC_BLOCK_SIZE_LESSER;
}
...
static void *NETMAN_rpc_handler(int fno, void *buffer, int size)
{
	...
	switch(fno)
	{
		...
		case NETMAN_IOP_RPC_FUNC_REG_NETWORK_STACK:
			...
			if(TxFrameBuffer == NULL) TxFrameBuffer = malloc(NETMAN_MAX_FRAME_SIZE * NETMAN_RPC_BLOCK_SIZE_LESSER);
			if(FrameBufferStatus == NULL) FrameBufferStatus = malloc(sizeof(struct NetManBD) * NETMAN_RPC_BLOCK_SIZE_LESSER);

			if(TxFrameBuffer != NULL && FrameBufferStatus != NULL)
			{
				EEFrameBufferStatus = ((struct NetManRegNetworkStack*)buffer)->FrameBufferStatus;
				memset(FrameBufferStatus, 0, sizeof(struct NetManBD) * NETMAN_RPC_BLOCK_SIZE_LESSER);
				memset(TxFrameBuffer, 0, NETMAN_MAX_FRAME_SIZE * NETMAN_RPC_BLOCK_SIZE_LESSER);
